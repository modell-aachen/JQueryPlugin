        ...
        var pagination = {
            ...
            createTemplate: function(args) {
                ...
                if (showPrevious || showPageNumbers || showNext) {
                    ...
                    html += '<nav class="pagination-container no-print">';
            ...
            go: function(number, callback) {
                ...
                // Need to start at 0 for Solr
                postData[alias.pageNumber ? alias.pageNumber : 'pageNumber'] = pageNumber - 1;
                ...
                formatAjaxParams.success = function(response) {
                    var targetclass = $(self.model.el).parent().attr('class').match(/\page_\d+/);
                    $(".responseData." + targetclass).html(response);
...
(function($) {
    $.sort = function(target) {
        ...
        if($(target).hasClass("headerSortUp")) {
            direction = " asc";
            sortList = [index +1,'headerSortDown'];
        }
        var sort = map[index] + direction;
        $.paginator(targetclass, sort, sortList);
        return false;
    }

    $.paginator = function(targetclass, sort, sortList) {
        var targetclass = targetclass || "";
        var sort = sort || "";
        var settings = $('div.jqPaginator' + targetclass);
        var data = {
                topic: settings.data("topic"),
                name: settings.data("name"),
                expand: settings.data("expand"),
                sort: sort
        };
        var total = parseInt(settings.data('total'));
        var limit = parseInt(settings.data('perpage'));
        var url = settings.data('url');
        if(settings.data('data')) {
            $.extend( data, settings.data('data'));
        }
        $('.renderPagination'+ targetclass).pagination({
            dataSource: url,
            ajax: {
                data: data,
                dataType: "html",
    ...
    $('.renderPagination').livequery(function() {
        var targetclass = "." + $(this).attr('class').match(/\page_\d+/);
        $.paginator(targetclass);
    } );
    $('.responseData th.header').livequery(function() {
        $('.responseData th.header').unbind().on('mousedown', function(elem){ $.sort($(elem.target));});
    }, function () {
        $(this).unbind();
    });
